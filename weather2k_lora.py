...
class LoRAInteraction(nn.Module):
    def __init__(self, N: int, H: int, r: int = 64, alpha: int = 64,
                 symmetric: bool = True, freeze_base: bool = True,
                 base_init: str = "eye"):  # "eye", "adj", or "zeros"
        super().__init__()
        self.N, self.H, self.r = N, H, r
        self.alpha = alpha
        self.symmetric = symmetric

        # Base E0 (frozen buffer)
        if base_init == "eye":
            E0 = torch.eye(N).expand(H, N, N).clone()
        elif base_init == "zeros":
            E0 = torch.zeros(H, N, N)
        else:
            # pass an external adjacency later via a setter if you want
            E0 = torch.eye(N).expand(H, N, N).clone()

        self.register_buffer("E0", E0)  # frozen by default
        if not freeze_base:
            self.E0 = nn.Parameter(self.E0)  # make it trainable if you want

        if symmetric:
            # E = E0 + (alpha/r) * (C @ C^T)
            self.C = nn.Parameter(torch.empty(H, N, r))
            nn.init.xavier_uniform_(self.C)
        else:
            # E = E0 + (alpha/r) * (B @ A)
            self.A = nn.Parameter(torch.empty(H, r, N))
            self.B = nn.Parameter(torch.empty(H, N, r))
            nn.init.kaiming_uniform_(self.A, a=math.sqrt(5))
            nn.init.zeros_(self.B)  # classic LoRA: zero-init up-proj so starts as identity (no update)

    def forward(self):
        if self.symmetric:
            # (H, N, r) @ (H, r, N) -> (H, N, N)
            update = torch.matmul(self.C, self.C.transpose(-1, -2))
        else:
            update = torch.matmul(self.B, self.A)  # (H,N,r) @ (H,r,N) -> (H,N,N)
        return self.E0 + (self.alpha / self.r) * update
...
class GraphAttentionLayer(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, n_heads: int, concat: bool = False, dropout: float = 0.4, leaky_relu_slope: float = 0.2):
        ...
        self.interact = LoRAInteraction(1866, n_heads, r=64, alpha=64, symmetric=False, base_init="eye")


        self.reset_parameters() # Reset the parameters
        # self.interaction_matrix_u = nn.Parameter(torch.ones(n_heads, 1866, 64))
        # self.interaction_matrix_v = nn.Parameter(torch.ones(n_heads, 64, 1866))
...
def train(rank, world_size, model, optimizer, hyperparameters, accumulation_steps, data_scaler, val_loader):
    ...
    if is_main():
        # Clean up
        prefix_str = 'weather2k_lora_gat_gru_traffic_pred'
        # Save the model
